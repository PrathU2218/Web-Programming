...
def user_panel(request):
    ...
    * won
    ...
    # Helpers
    all_distinct_bids = (
        Bid.objects.filter(user=request.user.id)
    ...
    selling = (
        Auction.objects.filter(closed=False, seller=request.user.id)
    ...
    sold = (
        Auction.objects.filter(closed=True, seller=request.user.id)
        .order_by("-publication_date")
        .all()
    )
    ...
    for auction in Auction.objects.filter(closed=True, id__in=all_distinct_bids).all():
        highest_bid = (
            Bid.objects.filter(auction=auction.id).order_by("-bid_price").first()
        )
...
def listing_page(request, auction_id):
    ...
    try:
        ...
        return render(
            request,
            "auctions/error_handling.html",
            {"code": 404, "message": "Auction id doesn't exist"},
        )
    ...
    if auction.closed:
        if highest_bid is not None:
            ...
            if request.user.id == auction.seller.id:
                return render(
                    request, "auctions/closed_no_offer.html", {"auction": auction}
                )
        ...
        if request.user.is_authenticated:
            watchlist_item = Watchlist.objects.filter(
                auction=auction_id, user=User.objects.get(id=request.user.id)
            ).first()
        ...
        if request.POST.get("on_watchlist") == "True":
            ...
            # Make sure it is not duplicated for current user
                ...
                return render(
                    request,
                    "auctions/error_handling.html",
                    {"code": 400, "message": "Auction is already on your watchlist"},
                )

        return HttpResponseRedirect("/" + auction_id)

    watchlist_auctions_ids = User.objects.get(id=request.user.id).watchlist.values_list(
        "auction"
    )
    watchlist_items = Auction.objects.filter(
        id__in=watchlist_auctions_ids, closed=False
    )
        ...
        if form.is_valid():
            ...
            if bid_price <= 0:
                return render(
                    request,
                    "auctions/error_handling.html",
                    {"code": 400, "message": "Bid price must be greater than 0"},
                )
            ...
            if auction.seller == user:
                return render(
                    request,
                    "auctions/error_handling.html",
                    {"code": 400, "message": "Seller cannot bid"},
                )

            # Check if current bid is the highest / else save new bid
            highest_bid = (
                Bid.objects.filter(auction=auction).order_by("-bid_price").first()
            )
            
            if (highest_bid is None or bid_price > highest_bid.bid_price) and bid_price > auction.current_price:
                # Add new bid to db
                ...
                return render(
                    request,
                    "auctions/error_handling.html",
                    {"code": 400, "message": "Youre bid is too small"},
                )
        else:
            return render(
                request,
                "auctions/error_handling.html",
                {"code": 400, "message": "Form is invalid"},
            )
    ...
    if category is not None:
        if category in [x[0] for x in categories_list]:
            ...
            return render(
                ...
                {"auctions": auctions, "category_full": category_full},
            )
        else:
            return render(
                request,
                "auctions/error_handling.html",
                {"code": 400, "message": "Category is incorrect"},
            )

    return render(
        request,
        "auctions/error_handling.html",
        {"code": 404, "message": "This page doesn not exist"},
    )
        ...
        if user is not None:
            ...
            if "next" in request.POST:
                return HttpResponseRedirect(
                    reverse("auctions:" + request.POST.get("next")[1:])
                )
...
def register(request):
    ...
    if request.method == "POST":
        ...
        # Ensure that username is entered
        if username is None or username == "":
            return render(
                request, "auctions/register.html", {"message": "Please enter username"}
            )

        # Ensure that email is entered
        if email is None or email == "":
            return render(
                request, "auctions/register.html", {"message": "Please enter email"}
            )

        # Ensure that passowrd is entered
        if password is None or password == "":
            return render(
                request, "auctions/register.html", {"message": "Please enter password"}
            )
